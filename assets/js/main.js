var gameState = {
	...
	init: function() {
		...
		while (i < gameState.elements.pokemonsEl.length) {
			...
			gameState.elements.pokemonsEl[i].onclick = function() {
				...
				// change name in battle screen

				var player1Name = (document
					.querySelector('.player')
					.querySelector('.player1')
					.querySelector('.stats')
					.querySelector('.name').innerHTML = gameState.userPokemon);

				var cpuName = (document
					.querySelector('.cpu')
					.querySelector('.player2')
					.querySelector('.stats')
					.querySelector('.name').innerHTML = gameState.rivalPokemon);

				console.log('my Pokemon name is: ' + gameState.userPokemon);
				console.log('Rival pokemon name is: ' + gameState.rivalPokemon);
	...
	checkWinner: function(enemy, attacker) {
		if (enemy.health <= 0) {
			...
			var winnerIs = (document.querySelector('.winner').innerHTML =
				'Winner is: ' + attacker.name);
			document.getElementById('playAgain').style.visibility = 'visible';
		}

		var again = document
			.getElementById('playAgain')
			.addEventListener('click', againBtn, false);
